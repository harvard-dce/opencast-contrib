...
export default class SocialDataPlugin extends DataPlugin {
  ...
  async read(_context, { videoId, operation }) {
    if (operation === 'canComment') {
      return await this.getCanComment();
    }
  ...
  async addComment(videoId, data) {
    ...
    // #DCE OPC-928, uncomment the following link to simulate HTTP 303 response
    // when testing locally. Local testing cannot use authenticated endpoints
    // return { success: false,  newLoc: 'https://www.harvard.edu?id=newMpdId'};
  }
