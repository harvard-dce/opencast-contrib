 ...
 */
/*
 * #DCE This is an override of UPV to support multi-video pubs in iPadOS v16
 * iPadOS v17 does not appear to have a problem with multi-video pubs.
 * TODO: Once iPadOSv16 becomes deprecated, remove this override
*/
import {
  Events,
  triggerEvent,
...
export function getHlsSupport(forceNative = false) {
  ...
  // #DCE iPadOSv16 support: forcing blanket native HLS for all iOS devices, but not non-iOS devices
  const isIOS = DceUtils.testIfIOS();
  if (isIOS)
  {
    return HlsSupport.NATIVE;
  }
  // end #DCE iPadOSv16 support. i.e. Don't force Android or non-IOS to use native HLS
  // This #DCE bug patch is only for iOS devices.
...
export class HlsVideo extends Mp4Video {
  ...
  async setQuality(q) {
    ...
    if (getHlsSupport(this.forceNative) === HlsSupport.MEDIA_SOURCE_EXTENSIONS) {
      ...
      this.player.log.warn(`Could not set video quality of HLS stream,
         because the HLS support of this browser is native.`
      );
    }
  }
