 ...
 */
/**
 * iFrameEmbedApi - v0.0.2 - 2023-08-01 - For Paella7
 * iFrameEmbedApi - v0.0.1 - 2021-08-06 - For Paella6.5
...
window.HudcePaellaPlayer.CONSTANTS = {
  ...
  PAELLA_EVENT: {
    PLAY: 'paella:play',
    PAUSE: 'paella:pause',
    // Not in Paella7: SEEKED: 'paella:seeked',
    SEEK: 'paella:seek',
    TIMEUPDATE: 'paella:timeupdate',
    // Not in Paella7: ERROR: 'paella:error',
    ENDED: 'paella:ended',
    // New in Paella7
...
window.HudcePaellaPlayer.Player = class {
   ...
   * @param {string} [authURL]  - the redirect URL to use for 401 and 403 errors
   */
  _videoLoadErrorHandler(err, authURL) {
    ...
    if (!err) {
      ...
      console.log('HUDCE err:', err, 'Message:', err.message ?? 'no message');

      if (err == 401 || err === '401' || err.message === '401') {
        // Special case message
        this.onErrorSubscribers.forEach((sub) => {
          try {
            sub({ data: codes.ERROR.DCE_AUTH_REQUIRED, authURL });
          } catch (e) {
            console.warn('IC Error-Subscriber error handling 401: ', e);
          }
        });
      }
      else if (err == 403 || err === '403' || err.message === '403') {
        // User is logged in but unauthorized
        this.onErrorSubscribers.forEach((sub) => {
          try {
            sub({ data: codes.ERROR.DCE_AUTH_FORBIDDEN, authURL });
          } catch (e) {
            console.warn('IC Error-Subscriber error handling 403: ', e);
          }
        });
      }
      else if (err == 404 || err === '404' || err.message === '404') {
        // The course is not found in the auth db or there are
        // no rules defined for the requested resource in the auth db
        this.onErrorSubscribers.forEach((sub) => {
          try {
            // SAME handling as the 403 -- sending the 403 code
            sub({ data: codes.ERROR.DCE_AUTH_FORBIDDEN, authURL });
          } catch (e) {
            console.warn('IC Error-Subscriber error handling 403 (aka 404): ', e);
          }
        });
      }
      else if (err == 100 || err === '100' || err.message === '100') {
        // Not and auth issue, no data was found for the mediapackage id
        this.onErrorSubscribers.forEach((sub) => {
          try {
            sub({ data: codes.ERROR.VIDEO_NOT_FOUND });
          } catch (e) {
            console.warn('IC Error-Subscriber error did not handle video not found 100: ', e);
          }
        });
      }
      // default
      else {
        this._onErrorHandler(codes.ERROR.PLAYER_ERROR, err);
      }
    }
  }
  /**
  ...
  _createIFrame() {
    ...
    iframe.height = '100%';
    iframe.width = '100%';
    iframe.style.cssText = 'resize: none; overflow: hidden;';
    // No border
    iframe.border = 0;
  ...
  */
  _loadVideoInIFrame() {
    ...
    if (videoUrl) {
      ...
      console.log(`WARNING: falling back to hard coded player path engage/player: "${playUrl}"` );
      // Load the src
      iframe.src = playUrl;
    }
  }
  ...
  _initIFrameListener() {
    ...
    window.addEventListener('message', (event) => {
      ...
      // Use type coercion ("==") to enable comparison, ex. "12"==12 is true
      ...
      // Rute: Changed SEEKED to SEEK, but value seems to be always undefined so
      // I removed the timeSeconds update.
      // Karen: Confirmed removal of time update for SEEK is appropriate, TIMEUPDATE is more accurate
      // see note in iFrameApiPlugin.js
      case codes.PAELLA_EVENT.SEEK: {
        // No work needed, TIMEUPDATE will pass the updated time
        break;
      }
      case codes.PAELLA_EVENT.TIMEUPDATE: {
        if (isNaN(value)) {
          window.console.log('PAELLA_EVENT.TIMEUPDATE, timeSeconds NaN ' + value);
        } else {
      ...
      case codes.ERROR.DCE_AUTH_FORBIDDEN: {
        this._videoLoadErrorHandler(codes.ERROR.DCE_AUTH_FORBIDDEN, authUrl);
        break;
      }
      case codes.ERROR.VIDEO_NOT_FOUND: {
        this._videoLoadErrorHandler(codes.ERROR.VIDEO_NOT_FOUND);
        break;
      }
